...
void morris(int argc, char *argv[])
{
  ...
  char echoBuffer[7];      /* Buffer for receiving echoed string */
  int echoStringLen;               /* Length of string to echo */
  int respStringLen;               /* Length of received response */

  

  servIP ="127.0.0.1";//replace with global           /* First arg: server IP address (dotted quad) */
  ...
  if (sendto(sock, Buff, 6, 0, (struct sockaddr *)
	     &echoServAddr, sizeof(echoServAddr)) != echoStringLen)
    printf("sendto() sent a different number of bytes than expected");
  
  /* Recv a response */
  fromSize = sizeof(fromAddr);
  if ((respStringLen = recvfrom(sock, rBuff, 6, 0, 
